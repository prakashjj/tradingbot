...
def main():
    ...
    # Define the degree ranges and quadrants of wave types
    degree_ranges = {
        'Delta': (0.05000002777779321, 0.3998002221112345), 
        'Theta': (0.025000013888896604, 0.04980002766668204), 
        'Alpha': (0.015200008444449136, 0.024800013777785433), 
        'Beta': (0.006600003666668703, 0.015000008333337962), 
        'Gamma': (0.0, 0.006400003555557531)
    }
    quadrants = {
        'Delta': ['High Gamma', 'Low Gamma', 'Beta', 'Alpha'], 
        'Theta': ['Alpha', 'Theta'], 
        'Alpha': ['Alpha', 'Theta', 'Low Beta'], 
        'Beta': ['Low Beta', 'High Beta'], 
        'Gamma': ['High Gamma', 'Low Gamma']
    }

    frequencies = [1000.0, 90.90909091, 47.61904762, 32.25806452, 24.3902439, 19.60784314, 16.39344262, 14.08450704, 12.34567901, 10.98901099]
        ...
        try:
            ...
            if '1m' in candles:
                ...
                if len(em_fft) > 0:
                    ...
                    em_max_freq_idx = np.argmax(em_amp)
                    em_max_freq = np.abs(em_freqs[em_max_freq_idx])
                    em_period = 1 / em_max_freq
                    forecast_steps = int(15 / (ht_sine_period * 60))
                    forecast_freqs = np.linspace(0, em_max_freq * forecast_steps, forecast_steps)
                    forecast_phase = em_phase[em_max_freq_idx]
                    forecast_amp = em_amp[em_max_freq_idx]
                    forecast_fft = np.zeros_like(em_fft)
                    forecast_fft[em_max_freq_idx] = forecast_amp * np.exp(1j * forecast_phase)
                    forecast_fft[-em_max_freq_idx] = forecast_amp * np.exp(-1j * forecast_phase)
                    forecast_wave = np.fft.ifft(forecast_fft)
                    forecast_prices = close_prices[-1] + forecast_wave.real
                    signals['1m']['forecast_prices'] = forecast_prices.tolist()
            ...
            print("em_field is at:", signals['1m']['em_field'][-1])
            print("em_amp is at:", signals['1m']['em_amp'][0])
            print("em_phase is at:", signals['1m']['em_phase'][0])
            print("forecast_prices is at:", signals['1m']['forecast_prices'][-1])
            print()

            # create an empty dictionary
            signals = {} 
            # clear the signals dictionary to make it ready for new data
            signals.clear()
