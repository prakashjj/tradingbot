        ...
        try:
            ...
            if '1m' in candles:
                ...
                if len(em_fft) > 0:
                    ...
                    # Calculate the frequency range for the EM field (in Hz)
                    freq_range = np.fft.fftfreq(len(em_fft), d=1/len(em_fft))
                    freq_range = np.abs(freq_range)
                    signals['1m']['em_freq_range'] = freq_range.tolist()

                    # Create a stationary filter for the EM field
                    em_hi = np.percentile(em_amp, 90)
                    em_lo = np.percentile(em_amp, 10)
                    signals['1m']['em_hi'] = em_hi
                    signals['1m']['em_lo'] = em_lo

                    # Print the frequency values for the EM field
                    print("EM Frequencies (Hz):")
                    for idx, freq in enumerate(freq_range):
                        if em_amp[idx] > em_hi or em_amp[idx] < em_lo:
                            print("{:.2f} ({:.2f})".format(freq, em_amp[idx]))
                        else:
                            print("{:.2f}".format(freq))

                    # Calculate the forecast for the close price
                    close_price = candles['1m']['close'][-1]
                    print("Close price: {:.2f}".format(close_price))
                    forecast_price = em_field[-1] * (ht_sine_max - ht_sine_min) + ht_sine_min
                    print("Forecast price: {:.2f}".format(forecast_price))
                else:
                    continue
