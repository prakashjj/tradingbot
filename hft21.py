...
def main():
    ...
    while True:
        ...
        # Clear signals for each iteration
        signals = {}
        ...
        try:
            ...
            if '1m' in candles:
                ...
                signals['1m'] = {'ema_slow': float(ema_slow), 'ema_fast': float(ema_fast)}
            ...
            if '1m' in candles:
                ...
                ht_sine_last = ht_sine[-1]
                ht_sine_pct_to_min = (ht_sine_last - ht_sine_min) / (ht_sine_max - ht_sine_min)
                ht_sine_pct_to_max = (ht_sine_max - ht_sine_last) / (ht_sine_max - ht_sine_min)
                signals['1m']['ht_sine_pct_to_min'] = float(ht_sine_pct_to_min)
                signals['1m']['ht_sine_pct_to_max'] = float(ht_sine_pct_to_max)

            # Calculate the EM Field and Amplitude for the 1m timeframe
            if '1m' in candles:
                close_prices = np.asarray(candles['1m']['close'])
                em_field = talib.HT_DCPHASE(close_prices)
                em_amplitude = talib.HT_DCPERIOD(close_prices)
                signals['1m']['em_field'] = float(em_field[-1])
                signals['1m']['em_amplitude'] = float(em_amplitude[-1])

            # Calculate the EMA crossover for the 1m timeframe
            if '1m' in signals and 'ema_slow' in signals['1m'] and 'ema_fast' in signals['1m']:
                ema_slow = signals['1m']['ema_slow']
                ema_fast = signals['1m']['ema_fast']
                if ema_fast > ema_slow and not trade_open:
                    signals['1m']['buy_signal'] = True
                    signals['1m']['sell_signal'] = False
                elif ema_fast < ema_slow and not trade_open:
                    signals['1m']['buy_signal'] = False
                    signals['1m']['sell_signal'] = True

            # Calculate the HT_SINE crossover for the 1m timeframe
            if '1m' in signals and 'ht_sine_pct_to_min' in signals['1m'] and 'ht_sine_pct_to_max' in signals['1m']:
                ht_sine_pct_to_min = signals['1m']['ht_sine_pct_to_min']
                ht_sine_pct_to_max = signals['1m']['ht_sine_pct_to_max']
                if ht_sine_pct_to_min > 0.5 and not trade_open:
                    signals['1m']['buy_signal'] = True
                    signals['1m']['sell_signal'] = False
                elif ht_sine_pct_to_max > 0.5 and not trade_open:
                    signals['1m']['buy_signal'] = False
                    signals['1m']['sell_signal'] = True

            # Calculate the EM Field and Amplitude crossovers for the 1m timeframe
            if '1m' in signals and 'em_field' in signals['1m'] and 'em_amplitude' in signals['1m']:
                em_field = signals['1m']['em_field']
                em_amplitude = signals['1m']['em_amplitude']
                if em_field > 0 and em_amplitude > 0 and not trade_open:
                    signals['1m']['buy_signal'] = True
                    signals['1m']['sell_signal'] = False
                elif em_field < 0 and em_amplitude > 0 and not trade_open:
                    signals['1m']['buy_signal'] = False
                    signals['1m']['sell_signal'] = True

            # Calculate the EMA crossover for the 3m timeframe
            if '3m' in candles:
