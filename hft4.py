...
def trigger():
    ...
    while True:
        ...
        candles = {}
        for interval in timeframes:
            tf_candles = client.futures_klines(symbol=TRADE_SYMBOL, interval=interval, startTime=start_time * 1000, endTime=end_time * 1000, cache_options=None)
            candles[interval] = []
            for candle in tf_candles:
                candles[interval].append({
                    'timestamp': candle[0],
        ...
        # Call the check_mtf_signal function with the updated data
        mtf = check_mtf_signal(candles, timeframes, mtf_signal)

        # Print the trade signal and other relevant information
        print(f"Trade signal: {mtf[0]}")
        print(f"Distance to min momentum: {mtf[1]:.2f}%")
        print(f"Distance to max momentum: {mtf[2]:.2f}%")
        print(f"Cycle time: {mtf[4]}")
        print(f"Time remaining: {mtf[5]}")
        print(f"Distance to min value: {mtf[6]:.2f}%")
        print(f"Distance to max value: {mtf[7]:.2f}%")
        print()

        # Update the mtf_signal variable based on the current mtf data
        if mtf[0] == "bullish" and mtf[6] <= 10:
            mtf_signal = "bullish"
            print("entry long")
            break
        elif mtf[0] == "bearish" and mtf[7] <= 10:
            mtf_signal = "bearish"
            print("entry short")
            break

        # Wait for some time before fetching new data and updating the signals
        time.sleep(5)

        # Delete the candles data to get fresh data in the next iteration
        del candles
